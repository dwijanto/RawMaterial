...
Public Class FormRawMaterialCT
    ...
    Private Sub getData(ByVal Dataset As DataSet, ByRef TabIndex As Integer)
        ...
        sqlstr = "select setval('rawmaterial',1,false);select nextval('rawmaterial') ||' '|| materialname as materialname from (select (rawmaterialname || ' (' || case when length(currency) > 0  then currency else 'No Crcy' end  || ')'  ) as materialname from rmmaterialraw where upper(sheetname) = '" & UCase(TabControl1.TabPages(TabIndex).Text) & "' order by keyid) as foo"
